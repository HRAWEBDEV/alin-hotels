...
import { type HotelRoomType } from '../../services/hotel-room-types/hotelRoomTypesApiActions';
import {
 ColumnDef,
 useReactTable,
 getCoreRowModel,
...
export default function OperatorsTable({ dic }: { dic: HotelsDictionary }) {
 ...
 const columns: ColumnDef<HotelRoomType>[] = useMemo(() => {
       ...
       <DropdownMenu dir={localeInfo.contentDirection}>
        ...
        <DropdownMenuContent align='end' onClick={(e) => e.stopPropagation()}>
         ...
         <DropdownMenuItem
          ...
          onClick={() => {
           const typedOriginal = row.original;
           onRemoveHotelRoomType(typedOriginal.id);
          }}
         >
  ...
  ] as ColumnDef<HotelRoomType>[];
 }, [dic, onRemoveHotelRoomType, localeInfo, onEditRoomType]);
  ...
  <div className='bg-background border border-input lg:rounded-es-none lg:rounded-ss-none rounded flex flex-col overflow-hidden'>
   <div className='p-1 border-b border-input flex justify-between items-center min-h-12 shrink-0'>
    <div className='flex gap-2'>
     ...
     <Button className='h-auto' onClick={() => onAddRoomType()}>
      <FaPlus />
