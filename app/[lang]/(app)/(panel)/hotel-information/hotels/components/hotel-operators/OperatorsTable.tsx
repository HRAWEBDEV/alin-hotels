...
import { type HotelOperator } from '../../services/hotel-operators/hotelOperatorsApiActions';
import {
 ColumnDef,
 useReactTable,
 getCoreRowModel,
...
export default function OperatorsTable({ dic }: { dic: HotelsDictionary }) {
 ...
 const validFilters = Object.keys(getValues()).filter(
  (key) => getValues()[key as keyof HotelOperatorSchema],
 );
 ...
 const columns: ColumnDef<HotelOperator>[] = useMemo(() => {
        ...
        <DropdownMenuContent align='end' onClick={(e) => e.stopPropagation()}>
         <DropdownMenuItem
          ...
          onClick={() => {
           ...
           onEditOperator(typedOriginal.id);
          }}
         >
         ...
         <DropdownMenuItem
          ...
          onClick={() => {
           const typedOriginal = row.original;
           onRemoveHotelOperator(typedOriginal.id);
          }}
         >
  ...
  ] as ColumnDef<HotelOperator>[];
 }, [dic, onRemoveHotelOperator, localeInfo, onEditOperator]);
  ...
  <div className='bg-background border border-input lg:rounded-es-none lg:rounded-ss-none rounded flex flex-col overflow-hidden'>
   <div className='p-1 border-b border-input flex justify-between items-center min-h-12 shrink-0'>
    <div className='flex gap-2'>
     ...
     <Button className='h-auto' onClick={() => onAddOperator()}>
      <FaPlus />
